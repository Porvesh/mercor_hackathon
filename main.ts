...
import { EnhancedPerformanceTracker } from './EnhancedPerformanceTracker';
...
async function main() {
	...
	async function frame() {
		...
		// Update performance tracker with the measured times
		const numParticles = sphFl ? sphSimulator.numParticles : mlsmpmSimulator.numParticles;
		performanceTracker.update(simulationTime, renderTime, numParticles);
		
		// If you want to slow down the rendering, you can add a delay here:
		// await new Promise(resolve => setTimeout(resolve, 100)); // Uncomment to slow down rendering
